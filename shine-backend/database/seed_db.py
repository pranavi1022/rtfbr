...
Two modes:
  seed_in_background()  — called at startup (non-blocking, silent)
  run_seed()            — called via /api/trigger-seed (blocking, returns full log)

The /api/trigger-seed endpoint returns JSON so you can see EXACTLY what happened.
"""
...
def run_seed(force: bool = False) -> dict:
    ...
    Returns a dict with 'log' (list of strings) and 'success' (bool).
    Set force=True to re-insert data even if tables already have rows.
    """
    log = []
    ...
    try:
        from config import DB_TYPE, DATABASE_URL
        if DB_TYPE != "postgresql":
            L("[SEED] Not PostgreSQL — skipping")
            return {"success": True, "log": log, "skipped": True}

        if not DATABASE_URL:
            L("[SEED] ERROR: DATABASE_URL not set")
            return {"success": False, "log": log}

        import psycopg2
        L(f"[SEED] Connecting to PostgreSQL...")
        conn = psycopg2.connect(DATABASE_URL, sslmode="require", connect_timeout=15)
        conn.autocommit = True   # Each statement commits immediately — no rollback risk
        cur = conn.cursor()
        L("[SEED] Connected OK")

        # ── Create tables ────────────────────────────────────────────────
        L("[SEED] Creating tables...")
        ...
        cur.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint WHERE conname = 'uq_project_title'
                ) THEN
                    ALTER TABLE projects ADD CONSTRAINT uq_project_title UNIQUE (title);
                END IF;
            END $$;
        """)
        ...
        cur.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint WHERE conname = 'uq_keyword'
                ) THEN
                    ALTER TABLE keywords ADD CONSTRAINT uq_keyword UNIQUE (keyword);
                END IF;
            END $$;
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_keywords_keyword ON keywords (LOWER(keyword))")
        L("[SEED] keywords table OK")
        ...
        cur.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint WHERE conname = 'uq_skill_type'
                ) THEN
                    ALTER TABLE skills ADD CONSTRAINT uq_skill_type UNIQUE (skill_name, project_type);
                END IF;
            END $$;
        """)
        L("[SEED] skills table OK")
        ...
        cur.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint WHERE conname = 'uq_skill_dep'
                ) THEN
                    ALTER TABLE skill_dependencies ADD CONSTRAINT uq_skill_dep UNIQUE (skill_name, depends_on);
                END IF;
            END $$;
        """)
        L("[SEED] skill_dependencies table OK")
        ...
        cur.execute("""
            DO $$ BEGIN
                IF NOT EXISTS (
                    SELECT 1 FROM pg_constraint WHERE conname = 'uq_resource'
                ) THEN
                    ALTER TABLE learning_resources ADD CONSTRAINT uq_resource UNIQUE (skill_name, resource_title);
                END IF;
            END $$;
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_lr_skill ON learning_resources (LOWER(skill_name))")
        L("[SEED] learning_resources table OK")
        ...
        L(f"[SEED] Current: {project_count} projects, {keyword_count} keywords")

        if not force and project_count >= 50 and keyword_count >= 80:
            L("[SEED] Already seeded — skipping. Use ?force=true to re-seed.")
            cur.close()
            conn.close()
            return {"success": True, "log": log, "skipped": True}

        # ── INSERT KEYWORDS (one by one to isolate errors) ────────────────
        L("[SEED] Inserting keywords...")
        keywords = [
        ...
        for kw in keywords:
            try:
                cur.execute(
                    "INSERT INTO keywords (keyword, category, domain) VALUES (%s,%s,%s) ON CONFLICT (keyword) DO UPDATE SET category=EXCLUDED.category, domain=EXCLUDED.domain",
                    kw
                )
                kw_ok += 1
            except Exception as e2:
                L(f"[SEED]   keyword SKIP '{kw[0]}': {e2}")
                kw_skip += 1
        L(f"[SEED] Keywords done: {kw_ok} inserted, {kw_skip} skipped")

        # ── INSERT PROJECTS ───────────────────────────────────────────────
        L("[SEED] Inserting projects...")
        projects = [
            ...
            ('Smart Energy Monitor','Current sensors measure appliance power. Real-time dashboard shows usage and cost.','IoT','Advanced','iot_project','Python,Flask,MQTT,PostgreSQL,React,Chart.js,Docker'),
            ...
            ('DeFi Lending Protocol','Decentralised lending/borrowing. Smart contracts handle collateral and interest.','Blockchain','Advanced','blockchain_project','Solidity,Hardhat,JavaScript,React,Web3.js,The Graph'),
        ...
        for pr in projects:
            try:
                ...
                L(f"[SEED]   project SKIP '{pr[0][:40]}': {e2}")
                pr_skip += 1
        L(f"[SEED] Projects done: {pr_ok} inserted, {pr_skip} skipped")

        # ── INSERT SKILLS ─────────────────────────────────────────────────
        L("[SEED] Inserting skills...")
        skills = [
        ...
        for sk in skills:
            try:
                ...
                L(f"[SEED]   skill SKIP '{sk[0]}/{sk[1]}': {e2}")
        L(f"[SEED] Skills done: {sk_ok}")

        # ── INSERT DEPENDENCIES ───────────────────────────────────────────
        L("[SEED] Inserting skill_dependencies...")
        deps = [
            ('React','JavaScript'),('React','HTML'),('React','CSS'),
            ('Flask','Python'),('PostgreSQL','Python'),
            ('React Native','JavaScript'),('Firebase','JavaScript'),
            ('Kotlin','Java'),('Three.js','JavaScript'),('Pygame','Python'),
        ...
        for d in deps:
            try:
                cur.execute(
                    "INSERT INTO skill_dependencies (skill_name, depends_on) VALUES (%s,%s) ON CONFLICT (skill_name, depends_on) DO NOTHING",
                    d
                )
                dep_ok += 1
            except Exception as e2:
                L(f"[SEED]   dep SKIP: {e2}")
        L(f"[SEED] Dependencies done: {dep_ok}")

        # ── INSERT LEARNING RESOURCES ──────────────────────────────────────
        L("[SEED] Inserting learning_resources...")
        resources = [
            ('HTML','HTML Foundations','https://developer.mozilla.org/en-US/docs/Learn/HTML','Beginner'),
            ('HTML','HTML Full Course','https://www.freecodecamp.org/learn/2022/responsive-web-design/','Beginner'),
            ('CSS','CSS Fundamentals','https://web.dev/learn/css','Beginner'),
            ('CSS','CSS Tricks Guide','https://css-tricks.com/guides/','Intermediate'),
            ...
            ('Pygame','Pygame Beginners Guide realpython','https://realpython.com/pygame-a-primer/','Beginner'),
        ...
        for r in resources:
            try:
                cur.execute(
                    "INSERT INTO learning_resources (skill_name, resource_title, resource_link, difficulty) VALUES (%s,%s,%s,%s) ON CONFLICT (skill_name, resource_title) DO UPDATE SET resource_link=EXCLUDED.resource_link, difficulty=EXCLUDED.difficulty",
                    r
                )
                res_ok += 1
            except Exception as e2:
                L(f"[SEED]   resource SKIP '{r[0]}/{r[1][:30]}': {e2}")
        L(f"[SEED] Resources done: {res_ok}")
        ...
        for tbl in ('projects','keywords','skills','skill_dependencies','learning_resources'):
            ...
            L(f"[SEED]   {tbl:30s} → {counts[tbl]} rows")

        cur.close()
        conn.close()
        L("[SEED] COMPLETE!")
        return {"success": True, "log": log, "counts": counts}

    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        L(f"[SEED] FATAL ERROR: {e}")
        L(tb)
        return {"success": False, "log": log, "error": str(e)}


def seed_in_background():
    """Non-blocking background seed for app startup."""
    t = threading.Thread(target=lambda: run_seed(force=False), daemon=True)
    t.start()
